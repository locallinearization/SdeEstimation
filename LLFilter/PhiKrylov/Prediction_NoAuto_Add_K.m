function [ms, Ps, ms1, Ps1] = Prediction_NoAuto_Add_K(tt,tt0,m0,P0,theta,FileName,rtol,atol)
%
% This function computes the first two conditional moments of the equation:
%   dx = f(x,t)*dt + sum(b(t)*dw),
% where, f(x)=A*x+a(t), a(t)=a0*t+a1, b(t)=b0*t+b1, A is a constant matrix, and
% a0, a1, b0 and b1 are vectors
% These moments satisface the equations:
%   dm/dt = A*m+a(t),  m(0)=m0
%   dP/dt = A*P + P*A' + a(t)*m' + m*a'(t)+ sum(b(t)*b'(t)), P(0)=P0
...
[f0,b,A,a1,b1] = FileName(tt0,m0,theta);
h=tt-tt0;
