...
% Model definition
Const_FitzhughNagumo;

load data_FitzhughNagumo  
Z=datos;
...
LB_T=[-5 -5 0];    % lower bound     
UB_T=[5 5 1];      % upper bound
ThetaIndex=[1 2 3];
...
Theta0=Theta;     
Theta0(ThetaIndex)=[ 2  -3  0.5];    % example of feasible parameter initial values
% Theta0(ThetaIndex)=[ -2  -3  0.5];   % example of unfeasible parameter initial values
...
switch OptimMethod
  ...
  case 'UMDA+fmincon'
    ...
    OptimOptions.PopSizeRate = 30;    % PopSizeRate * length(ParVar) = Population Size;
    OptimOptions.NumGen = 15;         % Number of Generations
...
switch InnovMethod
    ...
    case 'AdapDeterInnov'
       n_m = length(t);
       mpts = 50.*ones(1,n_m);    % maximum artificial points allowed between observation times
