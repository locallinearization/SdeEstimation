...
LB_T=[0.125  2.5  1  0.05  -0.01];
UB_T=[0.375  7.5  3  0.15   0.01];
ThetaIndex=[1 2 4];

% state initial values for local optimization
Xf0=X0;

% parameter initial value for local optimization
Theta0=Theta;     
Theta0(ThetaIndex)=[0.35 3 0.14];
...
% InnovMethod='AdapDeterInnov';     % with Adaptive Deterministic LL Filter
  InnovMethod='ExactInnov';         % with exact Linear filter of Minimum Variance 
switch InnovMethod
    case 'ClassInnov'
       n_m = length(t);
    ...
    case 'DeterInnov'
       n_m = length(t);
       mpts = 400.*ones(1,n_m);    % a priori artificial points between observation times
       EstimOptions = EstimSet(t,Z,Xf0,Pf0,Theta0,FileNames,LKWindow,nu_stast,LB,UB,OptimMethod,OptimOptions,mpts);
    case 'StochInnov'
       n_m = length(t);
       mpts = 32.*ones(1,n_m);   % a priori artificial points between observation times
       NumSim=[];
       AbsTol=[0,0,1e-3];
       RelTol=[0,0,1e-3];
    ...
    case 'ExactInnov'
       EstimOptions = EstimSet(t,Z,Xf0,Pf0,Theta0,FileNames,LKWindow,nu_stast,LB,UB,OptimMethod,OptimOptions,[],0,0);
    otherwise
