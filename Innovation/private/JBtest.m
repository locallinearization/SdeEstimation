function [H,P,JBSTAT,CV,Warning] = JBtest(x,alpha,mctol)
...
if ~isvector(x) || ~isreal(x)
    error(message('stats:jbtest:BadInput'));
end
...
ssize = length(x);
if ssize < 2
    error(message('stats:jbtest:NotEnoughData'));
elseif ssize == 2
...
elseif ~isscalar(alpha) || ~(0 < alpha && alpha < 1)
    error(message('stats:jbtest:BadAlpha'));
end
...
if nargin < 3 || isempty(mctol)
   ...
   if ~isscalar(mctol) || mctol<=0
      error(message('stats:jbtest:BadStdErrTol'));
   end
end
...
if isempty(mctol)
    ...
    if (alpha < alphas(1)) || (alphas(end) < alpha)
        error(message('stats:jbtest:OutofRangeAlpha', sprintf( '%g', alphas( 1 ) ),...
    ...
    if nargout > 1
        ...
        elseif (JBSTAT == CVs(1))
            P = alphas(1);
        elseif isnan(JBSTAT)
            P = 0;  % Inf data gives NaN; declare highly significant
        else
...
H = double(JBSTAT > CV) + isnan(JBSTAT);
