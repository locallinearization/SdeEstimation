...
% Model definition
Const_DoublePotential;

load data_DoublePotential
Z=datos;
...
switch InnovMethod
    ...
    case 'DeterInnov'
       n_m = length(t);
       mpts = 200.*ones(1,n_m);    % a priori artificial points between observation times
       EstimOptions = EstimSet(t,Z,Xf0,Pf0,Theta0,FileNames,LKWindow,nu_stast,LB,UB,OptimMethod,OptimOptions,mpts);
    case 'StochInnov'
       n_m = length(t);
       mpts = 60.*ones(1,n_m);   % a priori artificial points between observation times
       NumSim=[];
       AbsTol=[0,0,0.001];
       RelTol=[0,0,0.001];
